...
func (t *Wrapper) injectedRead(res tunInjectedRead, outBuffs [][]byte, sizes []int, offset int) (n int, err error) {
	...
	// Seed reverse-flow state for injected (netstack-originated) outbound
	// packets so the inbound reply is admitted by RunIn even when no ACL
	// rule matches the reply's source. The normal OS-read path does this
	// via filterPacketOutboundToWireGuard; injected packets skip it.
	// Workaround for tailscale/tailscale#20064.
	if filt := t.filter.Load(); filt != nil {
		filt.RunOut(p, t.filterFlags)
	}
